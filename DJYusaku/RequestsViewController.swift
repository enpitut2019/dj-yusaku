...
class RequestsViewController: UIViewController {
    ...
    @objc func handleNowPlayingItemDidChangeOnListener(notification: NSNotification){
        guard let nowPlyingIndex = notification.userInfo!["nowPlayingIndex"] as? Int else { return }
        RequestsViewController.self.indexOfNowPlayingItemOnListener = nowPlyingIndex
        DispatchQueue.main.async {
            self.tableView.reloadData()
        }
    }
